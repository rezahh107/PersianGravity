...
if (JSON.stringify(enabledProduction?.fields) !== JSON.stringify(enabled.exact?.fields)) failures.push('production adapter output differs from qualified prototype output');
if (JSON.stringify(disabledProduction?.fields) !== JSON.stringify(disabled.exact?.fields)) failures.push('disabled production output differs from exact native output');
...
if (JSON.stringify(enabledMachine) !== JSON.stringify(disabledMachine)) failures.push('presentation digit shaping changed stable DOM attributes, links, form controls or machine values');
for (const [label, machine] of [['enabled', enabledProduction?.machine], ['disabled', disabledProduction?.machine]]) {
  ...
  if (!nonce || typeof nonce.value_property !== 'string' || nonce.value_property === '' || /[۰-۹]/.test(nonce.value_property)) failures.push(`${label} request nonce was missing or no longer native ASCII/security text`);
}
if (enabledProduction?.digit_script_count !== 1 || disabledProduction?.digit_script_count !== 0 || englishProduction?.digit_script_count !== 0) failures.push('digit presentation adapter activation did not stay bounded to enabled Persian UI');
if (enabledProduction?.visible_status_fields?.some((value) => /[0-9]/.test(value))) failures.push('enabled Persian workflow-info still contains ASCII digits');
if (typeof enabledProduction?.fields?.submitted !== 'string' || enabledProduction.fields.submitted.includes('11:59') || !enabledProduction.fields.submitted.includes('۱۱:۵۹')) failures.push('Submitted mixed-digit regression remains visible');
if (typeof enabledProduction?.fields?.last_updated !== 'string' || enabledProduction.fields.last_updated.includes('12:01') || !enabledProduction.fields.last_updated.includes('۱۲:۰۱')) failures.push('Last Updated mixed-digit regression remains visible');
if (typeof englishProduction?.fields?.submitted !== 'string' || !englishProduction.fields.submitted.includes('11:59') || englishProduction.fields.submitted.includes('۱۱:۵۹')) failures.push('English/non-Persian Submitted time did not remain ASCII');
if (typeof englishProduction?.fields?.entry_id !== 'string' || !/[0-9]/.test(englishProduction.fields.entry_id) || /[۰-۹]/.test(englishProduction.fields.entry_id)) failures.push('English/non-Persian visible Entry ID did not remain ASCII');
if (enabledProduction?.candidateEvidence?.due_getter_calls !== disabledProduction?.candidateEvidence?.due_getter_calls || enabledProduction?.candidateEvidence?.expiration_getter_calls !== disabledProduction?.candidateEvidence?.expiration_getter_calls) failures.push('production adapter changed operational due/expiration getter invocation counts');
if (enabledProduction?.candidateEvidence?.nested_due_getter_calls !== 0 || enabledProduction?.candidateEvidence?.nested_expiration_getter_calls !== 0 || disabledProduction?.candidateEvidence?.nested_due_getter_calls !== 0 || disabledProduction?.candidateEvidence?.nested_expiration_getter_calls !== 0) failures.push('production adapter nested operational getter through date_i18n');
if (enabledProduction?.candidateEvidence?.unrelated_wp_date_i18n !== disabledProduction?.candidateEvidence?.unrelated_wp_date_i18n || enabledProduction?.candidateEvidence?.unrelated_gf_format_date !== disabledProduction?.candidateEvidence?.unrelated_gf_format_date) failures.push('production adapter changed unrelated WordPress/Gravity Forms date formatting');

const persianDigits = '۰۱۲۳۴۵۶۷۸۹';
const asciiDigits = (value) => typeof value === 'string' ? value.replace(/[۰-۹]/g, (digit) => String(persianDigits.indexOf(digit))) : value;
...
if (enabled.exact?.candidateEvidence?.nested_due_getter_calls !== 0 || enabled.exact?.candidateEvidence?.nested_expiration_getter_calls !== 0) failures.push('presentation marker path re-entered an operational due/expiration getter');
...
for (let index = 0; index < fields.length; index += 1) {
  ...
  if (typeof enabledVisible !== 'string' || !enabledVisible.includes(observation.jalali)) failures.push(`enabled ${key}: visible output did not consume Jalali date`);
  const reconstructedNative = asciiDigits(enabledVisible.replace(observation.jalali, observation.native_fallback));
  if (reconstructedNative !== disabledVisible) failures.push(`enabled ${key}: native time/host formatting was not preserved around date replacement`);
  const utcCivil = new Date(Number(observation.timestamp_with_offset) * 1000).toISOString().slice(0, 19).replace('T', ' ');
  if (utcCivil !== observation.local_civil) failures.push(`enabled ${key}: timestamp-with-offset was not interpreted as local civil via UTC components`);
}
...
if (JSON.stringify(productionCivils) !== JSON.stringify(prototypeCivils)) failures.push(`production timestamp-with-offset civil dates differ from qualified prototype: ${JSON.stringify({ productionCivils, prototypeCivils })}`);
if (prototypeCivils[0] !== '2030-03-20 23:59:00' || prototypeCivils[1] !== '2030-03-21 00:01:00' || prototypeCivils[2] !== '2030-03-21 00:03:00' || prototypeCivils[3] !== '2030-03-21 23:59:00') failures.push(`Tehran local-midnight boundary fixture drifted: ${JSON.stringify(prototypeCivils)}`);
...
if (JSON.stringify(canonicalCandidate(enabledState)) !== JSON.stringify(canonicalCandidate(disabledState))) failures.push('Entry Detail module state changed raw/query/workflow/deadline/expiration state');
if (!/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/.test(enabledState.candidate_entry.gfapi.date_created)) failures.push('raw date_created stopped being canonical ASCII');
if (enabledState.candidate_entry.gfapi.date_created !== enabledState.candidate_entry.database.date_created || enabledState.candidate_entry.gfapi.date_created !== enabledState.candidate_entry.rest.date_created) failures.push('candidate DB/GFAPI/REST date_created mismatch');
if (enabledState.candidate_entry.gfapi.workflow_timestamp !== enabledState.candidate_entry.rest.workflow_timestamp) failures.push('candidate GFAPI/meta/REST workflow_timestamp mismatch');
if (enabledState.candidate_entry.due_timestamp === enabledState.candidate_entry.expiration_timestamp) failures.push('due and expiration timestamps are not distinct');
if (!enabledState.csv.marker_absent || !disabledState.csv.marker_absent || enabledState.csv.sha256 !== disabledState.csv.sha256) failures.push('CSV/export isolation changed or marker leaked');

const enabledGetter = enabled.exact?.candidateEvidence ?? {};
const disabledGetter = disabled.exact?.candidateEvidence ?? {};
if (enabledGetter.due_getter_calls !== disabledGetter.due_getter_calls || enabledGetter.expiration_getter_calls !== disabledGetter.expiration_getter_calls) failures.push('operational due/expiration getter invocation counts changed with presentation enabled');
if (disabledGetter.nested_due_getter_calls !== 0 || disabledGetter.nested_expiration_getter_calls !== 0) failures.push('disabled mode unexpectedly nested operational getters in date_i18n');

const entryIdAscii = String(enabledState.candidate_entry.gfapi.id);
const enabledLinks = enabledProduction?.machine?.links ?? [];
if (!enabledLinks.some((link) => typeof link.href_attribute === 'string' && link.href_attribute.includes(`lid=${entryIdAscii}`))) failures.push('visible Entry ID link did not retain the native ASCII entry-id query value');
if (enabledLinks.some((link) => /[۰-۹]/.test(String(link.href_attribute)) || /[۰-۹]/.test(String(link.href_property)))) failures.push('Persian digit glyphs leaked into workflow-info URLs/query parameters');
if (JSON.stringify(enabledProduction?.direction) !== JSON.stringify(disabledProduction?.direction) || enabledProduction?.direction?.computed !== 'rtl') failures.push('digit shaping changed or failed the existing Persian RTL/BiDi direction contract');
...
const result = {
  ...
  evidence_class: 'G008_ENTRY_DETAIL_AND_TIMELINE_PRODUCTION_RECONCILIATION',
  ...
  timeline_surfaces: ['gravityflow.timeline.initial-event', 'gravityflow.timeline.stored-events'],
  ...
  native_time_preserved: !failures.some((failure) => failure.includes('native time')),
...
if (failures.length) throw new Error(`Entry Detail/Timeline production qualification failed: ${failures.join('; ')}`);
console.log('G008_ENTRY_DETAIL_TIMELINE_PRODUCTION_RECONCILIATION PASS');
