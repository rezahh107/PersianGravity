...
The original Gravity Forms seam remains `gform_entries_field_value` for `date_created`. Exact Gravity Flow 3.1.0 additionally admits Inbox `date_created`/`last_updated`/`due_date` and Status-table `date_created`/`workflow_timestamp`. These adapters read authoritative raw sources and alter only presentation; DB/GFAPI/REST values, workflow/assignment state, Inbox due-date deadline/overdue state, Status query/sort/filter behavior, Inbox raw compare/sort/filter values, and Status CSV/export remain native. The remaining exact Flow 3.1.0 Status due-date, Entry Detail due/schedule/expiration, Timeline/history and dependent Print candidates are final no-admission for this version and remain native because exact source/runtime evidence does not expose a supported bounded date-only presentation seam. Full expansion evidence is in [`docs/G008_SYSTEM_DATE_EXPANSION.md`](docs/G008_SYSTEM_DATE_EXPANSION.md); converter provenance/range details remain in [`docs/G008_JALALI_PRESENTATION.md`](docs/G008_JALALI_PRESENTATION.md).
