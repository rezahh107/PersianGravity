...
- `jalali_presentation` — opt-in Jalali presentation of explicitly admitted authoritative Gregorian/system dates; current verified surfaces are Gravity Forms Entries List `date_created` plus exact Gravity Flow 3.1.0 Inbox `date_created`/`last_updated`/`due_date` and Status `date_created`/`workflow_timestamp`, without changing stored/native or workflow-owned operational values.
...
The original Gravity Forms seam remains `gform_entries_field_value` for `date_created`. Exact Gravity Flow 3.1.0 additionally admits Inbox `date_created`/`last_updated`/`due_date`, Status-table `date_created`/`workflow_timestamp`, and the Entry Detail workflow-info family Submitted / Last Updated / Due / Expiration through a bounded composed `gravityflow_date_format_entry_detail` + exact-marker `date_i18n` mechanism. These adapters alter only presentation; DB/GFAPI/REST values, workflow/assignment state, deadline/overdue/expiration state, query/sort/filter behavior, raw compare values, operational getter counts, and CSV/export remain native. In `fa_IR`, the same opt-in presentation module also shapes ASCII digits to Persian glyphs only in rendered `.gravityflow-status-box-field` text nodes inside the exact Entry Detail status box. Entry ID link/query values, element attributes, `data-*`, IDs, form/control values, APIs and stored workflow values remain native ASCII; non-Persian locale and module-disabled requests do not load the digit adapter. This glyph-only behavior is separate from calendar conversion: Entry Detail Scheduled may receive Persian digit glyphs when it is visibly rendered in the same status box, while its calendar/date seam remains `FINAL_NO_ADMISSION_FOR_EXACT_3_1_0`. Timeline/history and Print do not receive digit shaping. Exact Flow 3.1.0 Status due-date, Entry Detail Scheduled calendar conversion, Timeline initial/stored event dates, and any independent Print date seam remain evidence-qualified final no-admission; Print reuses field-grid/Timeline rendering but does not render the workflow sidebar. Full expansion evidence is in [`docs/G008_SYSTEM_DATE_EXPANSION.md`](docs/G008_SYSTEM_DATE_EXPANSION.md); converter provenance/range details remain in [`docs/G008_JALALI_PRESENTATION.md`](docs/G008_JALALI_PRESENTATION.md).
