...
**وضعیت provider همچنان جزئی و source-bounded است:** targetهای مالک GF `3.1.1.1`، Flow `3.1.0` و GravityView `3.3.4` هستند. Gravity Forms اکنون دقیقاً یک content admission محدود برای surface `gravityforms::frontend_runtime::shortcode:gravityform` دارد: `41` identity از source census مستقل `4207`. این catalog از baseline مرورشدهٔ WU-003 با SHA-256 `2c7960c895216da61ff7d6b8f6a248c2ac130fdf7927efc5c3d09c5e58bc79b1` منشأ گرفته و هفت اصلاح معنایی صریح PR #15 روی subset نهایی اعمال شده است؛ این وضعیت ترجمهٔ کامل Gravity Forms نیست. GravityView `3.3.4` اکنون در لایهٔ source admission با `source_status=PACKAGE_INSPECTED_METADATA_ONLY` و package SHA-256 `af5959fb6bf0cfcb4d07d14b1933cf9ea0a9d0f994b9991f27aed47edbcb9829` ثبت شده است؛ `project_source_authority=OWNER_SUPPLIED_EXACT_PACKAGE` و `vendor_authenticity=NOT_PROVEN` است و چون vendor POT در بسته وجود ندارد `vendor_pot_sha256=null` باقی می‌ماند. census اصلی `gk-gravityview` شامل `3127` identity canonical، `127` contextual، `42` plural و `3931` source reference است. `gk-query-filters` مرز dependency/domain جداگانه باقی می‌ماند و PersianGravity آن را manifest یا runtime-activate نمی‌کند. GravityView شش surface source-backed به registry اضافه می‌کند و مجموع registry فعلی `19` surface است: 6 Gravity Forms + 7 Gravity Flow + 6 GravityView. با وجود source admission، GravityView همچنان content-dormant است: `0` ترجمهٔ production، `0` runtime script handle و بدون provider MO، `.l10n.php` یا translation JSON. evidence مربوط به RTL/BiDi فقط source-based است و real licensed browser/UI validation همچنان `NOT_RUN` است. WU-004 فقط source admission است و translation-content admission نیست. برای Gravity Flow، runtime content authority دقیقاً دو surface مرورشده دارد: Inbox (`255` identity) و Status (`43` identity) با `10` identity مشترک یکسان؛ union نهایی `288` identity یکتا از source census مستقل `1098` است. identityهای admit‌نشده fallback موجود upstream/vendor/TranslationsPress و سپس متن منبع را حفظ می‌کنند؛ browser/licensed integration همچنان اثبات نشده است.
