...
final class PGR_Gravity_Flow_Inbox_Jalali_Presentation_Adapter {
	...
	/**
	 * Admit only the exact Gravity Flow runtime version already owned by the
	 * repository's production product registry. Missing/drifted authority is
	 * native fallback, never an inferred compatibility claim.
	 *
	 * @return bool
	 */
	private function is_exact_supported_host() {
		if ( ! defined( 'GRAVITY_FLOW_VERSION' ) || ! defined( 'PGR_PATH' ) ) {
			return false;
		}
		...
		$target   = isset( $products['gravityflow']['target_version'] ) ? (string) $products['gravityflow']['target_version'] : '';

		return '' !== $target && (string) GRAVITY_FLOW_VERSION === $target;
	}
}
