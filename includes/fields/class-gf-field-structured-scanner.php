...
final class PGR_GF_Field_Structured_Scanner extends GF_Field {
	...
	public function get_form_editor_field_settings() {
		return array( 'label_setting', 'description_setting', 'css_class_setting', 'label_placement_setting', 'pgr_scanner_setting' );
	}
	...
	/** Return the Form Editor integration script. @return string */
				...
				'outputs' => array_values(
					array_map(
						static function ( $output ) {
							return array( 'key' => $output['key'], 'label' => $output['label'] );
						},
						$profile['outputs']
					)
				),
		...
		return "
			...
			(function($) {
				...
				function mappingObject() {
					if (!field || !Array.isArray(field.scanner_mappings)) { return null; }
					var keys = outputKeys();
					if (field.scanner_mappings.length > keys.length) { return null; }
					var map = {};
					keys.forEach(function(key, index) { var value = field.scanner_mappings[index]; if (value !== undefined && value !== null && value !== '') { map[key] = value; } });
					return map;
				}
				...
				function fillSelect(select, selected, fields) {
					...
					Object.keys(fields).forEach(function(id) { if (supported(fields[id])) { select.append($('<option>', { value: id, text: format(messages.fieldOption, id, fields[id].label || fields[id].adminLabel || fields[id].type) })); } });
					if (selected && !optionExists(select, selected)) { select.append($('<option>', { value: selected, text: format(messages.invalidTarget, selected), selected: true })); }
					select.val(selected || '');
				}
				function ensureProfileOption() {
					var select = $('#pgr_scanner_profile'); var id = field && field.scanner_profile ? String(field.scanner_profile) : '';
					if (!id || optionExists(select, id)) { return; }
					var current = profiles[id]; var label = current ? current.label : id;
					select.append($('<option>', { value: id, text: label + ' — ' + messages.unavailableSuffix }));
				}
				function renderMappingRows(map, fields) {
					var list = $('[data-pgr-scanner-mapping-list]'); list.empty();
					outputs().forEach(function(output) {
						var row = $('<div>', { class: 'pgr-scanner-editor-mapping-row', 'data-pgr-output': output.key });
						var label = $('<label>', { class: 'pgr-scanner-editor-mapping-label' });
						label.append($('<span>', { text: output.label })); label.append($('<code>', { class: 'pgr-scanner-editor-mapping-key', dir: 'ltr', text: output.key }));
						var select = $('<select>', { 'data-pgr-scanner-mapping': output.key, 'aria-label': messages.destination + ': ' + output.label });
						fillSelect(select, map && map[output.key] ? String(map[output.key]) : '', fields);
						select.on('change', function() { window.PGRScannerEditor.setMapping(output.key, this.value); }); row.append(label, select); list.append(row);
					});
				}
				function validate(map, fields) {
					var warnings = []; var current = profile(); var used = {};
					if (!current || current.enabled !== true) { addWarning(warnings, format(messages.unavailableProfile, field && field.scanner_profile ? String(field.scanner_profile) : '')); }
					if (map === null) { addWarning(warnings, messages.invalidMapping); map = {}; }
					outputKeys().forEach(function(key) {
						var raw = map[key]; if (raw === undefined || raw === null || raw === '') { return; }
						var id = String(raw).trim();
						if (!/^[1-9]\\d*$/.test(id)) { addWarning(warnings, messages.invalidMapping); }
						else if (String(field.id) === id) { addWarning(warnings, messages.selfTarget); }
						else { if (used[id]) { addWarning(warnings, messages.duplicateTarget); } used[id] = true; if (!fields[id]) { addWarning(warnings, messages.missingTarget); } else if (!supported(fields[id])) { addWarning(warnings, messages.unsupportedTarget); } }
					});
					var box = $('#pgr_scanner_mapping_warning'); box.prop('hidden', warnings.length === 0); box.find('p').text(warnings.join(' '));
				}
				...
				window.PGRScannerEditor = {
					setProfile: function(value) { if (!field) { return; } SetFieldProperty('scanner_profile', value); SetFieldProperty('scanner_mappings', []); field.scanner_profile = value; field.scanner_mappings = []; refresh(); },
					setMapping: function(key, value) { if (!field) { return; } var keys = outputKeys(); var map = mappingObject() || {}; if (value === '') { delete map[key]; } else { map[key] = value; } var stored = keys.map(function(output) { return Object.prototype.hasOwnProperty.call(map, output) ? map[output] : ''; }); SetFieldProperty('scanner_mappings', stored); field.scanner_mappings = stored; refresh(); }
				};
	...
	/**
	 * Render a transient Persian-first Scanner component.
	 *
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$initial_status   = $is_operational ? 'ready' : 'invalid';
	...
	/** Structured Scanner never contributes data to Entry persistence. @return string */
	...
	private function runtime_mappings() {
		...
		foreach ( $this->scanner_mappings as $index => $target_id ) {
			if ( ! is_int( $index ) || ! isset( $outputs[ $index ] ) ) { return null; }
			if ( null === $target_id || '' === $target_id ) { continue; }
			if ( ! is_scalar( $target_id ) ) { return null; }
			$mappings[ $outputs[ $index ] ] = (string) $target_id;
		}
		return $mappings;
	}

	/** @param array $form Current form. @return array<int,array{id:string,type:string,displayOnly:bool}> */
