<?php
/** Static bilingual Help & Documentation catalog. @package PersianGravityForms */
...
final class PGR_Help_Catalog {
	...
	public static function all() {
		return array(
			...
			'scope' => self::topic(
				'دامنه و محدودیت‌ها', '<p>PersianGravity مالک Gravity Flow workflow، GravityView، منطق SRWF، font delivery، payment gateway، online Sayad inquiry، OCR/camera scanner، custom database یا ترجمه افزونه‌های دیگر نیست. Module Manager نیز extension marketplace یا third-party runtime registry نیست.</p>',
				'Scope & Limits', '<p>PersianGravity does not own Gravity Flow workflow, GravityView, SRWF business logic, font delivery, payment gateways, online Sayad inquiry, OCR/camera scanning, custom databases, or translations for other plugins. Module Manager is not an extension marketplace or third-party runtime registry.</p>'
			),
		);
	}
	...
	private static function topic( $fa_title, $fa_body, $en_title, $en_body ) {
		return array(
			'fa' => array( 'title' => $fa_title, 'body' => $fa_body, 'summary' => wp_strip_all_tags( $fa_body ) ),
			'en' => array( 'title' => $en_title, 'body' => $en_body, 'summary' => wp_strip_all_tags( $en_body ) ),
		);
	}
}
