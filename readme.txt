...
Owner-authorized targets are Gravity Forms 3.1.1.1, Gravity Flow 3.1.0 and GravityView 3.3.4. GravityView 3.3.4 is now source-admitted at `PACKAGE_INSPECTED_METADATA_ONLY` from the exact owner-supplied package SHA-256 `af5959fb6bf0cfcb4d07d14b1933cf9ea0a9d0f994b9991f27aed47edbcb9829`; `project_source_authority=OWNER_SUPPLIED_EXACT_PACKAGE`, `vendor_authenticity=NOT_PROVEN`, and the exact package contains no vendor POT so `vendor_pot_sha256=null`. Its primary `gk-gravityview` census is 3127 canonical identities, including 127 contextual and 42 plural identities, with 3931 source references. `gk-query-filters` remains a separate bounded Composer dependency/domain that PersianGravity does not manifest or runtime-activate. GravityView contributes six source-backed surfaces, making the current registry 19 total: 6 Gravity Forms + 7 Gravity Flow + 6 GravityView. GravityView remains translation-content dormant with zero production translated identities, zero runtime script handles, and no provider MO, `.l10n.php`, or translation JSON; RTL/BiDi evidence is source-only and real licensed GravityView browser/UI validation remains `NOT_RUN`. WU-004 is source admission only, not translation-content admission. Gravity Forms has exactly one bounded production content-admission record for `gravityforms::frontend_runtime::shortcode:gravityform`: 41 identities from the separate 4207-message source census. The WU-003 full reviewed baseline remains provenance for that subset; seven explicit PR #15 semantic corrections define the final admitted Persian content. Gravity Flow retains exactly two bounded production content-admission records: Inbox (255 identities) and Status (43 identities), with 10 identical overlaps deduplicated to an aggregate of 288 unique admitted identities from a separate 1098-message source census. Non-admitted identities retain the existing upstream/vendor/TranslationsPress/source-English fallback. This is partial surface admission, not full-product or browser-complete localization. See docs/LOCALIZATION.md and docs/ARCHITECTURE_CONTENT_ADMISSION_V2.md for the source/load-order and content-authority boundaries.
