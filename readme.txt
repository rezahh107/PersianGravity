...
Current repository source exposes six bounded source-defined modules enabled by default for backward compatibility plus one bounded opt-in module:
...
The module uses a source-owned Borkowski-lineage Gregorian→Jalali engine and typed `DateTimeInterface` presentation facade. Gravity Forms Entries List `date_created` remains admitted through `gform_entries_field_value`. Exact Gravity Flow 3.1.0 additionally admits Inbox `date_created`/`last_updated`/`due_date`, Status-table `date_created`/`workflow_timestamp`, Entry Detail workflow-info Submitted / Last Updated / Due / Expiration, and Timeline/history through bounded host presentation seams. Exact GravityView 3.3.4 admits only `date_created` and `date_updated` through their field-specific output hooks, using the authoritative raw UTC Entry properties and failing closed to native output for non-Persian, disabled, malformed-context, or version-drift states. Print owns no independent calendar or digit-shaping engine: its Timeline path inherits the already verified Timeline renderer. The same exact-source/caller-chain-bounded Timeline adapter changes only the localized Gregorian date component and, in a separate row-local context under `fa_IR`, shapes only visible host time digits (for example `12:01` → `۱۲:۰۱`) without changing the time value or timezone semantics. Native storage, raw `date_created`, note/event identity, bodies, ordering, API/state behavior and workflow semantics remain host-owned and unchanged. Unsupported locale, formats or exact version/source/caller-chain drift fail closed to native output; English/LTR Timeline and inherited Print remain native Gregorian with ASCII digits. Exact Flow 3.1.0 Status `due_date` and Entry Detail Scheduled calendar conversion remain evidence-qualified final no-admission surfaces. Separately, under `fa_IR`, exact Flow 3.1.0 Entry Detail status-box field text may receive presentation-only ASCII→Persian digit shaping; attributes, Entry ID URLs/query values, form/control values and all raw/system values remain native, while non-Persian and module-disabled requests do not load that Entry Detail digit adapter. Scheduled text may receive glyph shaping when present in that same status box, but its calendar conversion remains unadmitted. Timeline/history and Print remain outside the Entry Detail digit adapter and use their own narrower Timeline time seam.
...
The locked Gravity products are now fully admitted at the exact source versions: Gravity Forms 3.1.1.1 at 4207/4207 accepted source-backed identities, Gravity Flow 3.1.0 at 1098/1098, and GravityView 3.3.4 at 3127/3127. GravityView preserves the existing 461-identity historical surface union and adds the exact reviewed 2666-identity product remainder. One exact singular/plural gettext collision projects the 3127 admitted GravityView source identities to 3126 runtime provider entries; both source identities remain separately reviewed and authority-bound. Native product script maps remain empty and no provider translation JSON is generated.
