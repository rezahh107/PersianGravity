...
* Jalali System-Date Presentation (`jalali_presentation`) — opt-in; verified presentation covers Gravity Forms Entries List `date_created` plus exact Gravity Flow 3.1.0 Inbox `date_created`/`last_updated`/`due_date`, Status `date_created`/`workflow_timestamp`, Entry Detail Submitted / Last Updated / Due / Expiration, and Timeline/history. In the admitted `fa_IR` Timeline presentation, visible host-owned time digits are shaped to Persian glyphs; Print inherits both the verified Timeline calendar rendering and that bounded time-digit presentation. Stored/native timestamps, timezone semantics and workflow-owned operational values remain unchanged.
