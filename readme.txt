...
Owner-authorized targets are Gravity Forms 3.1.1.1, Gravity Flow 3.1.0 and GravityView 3.3.4. Exact owner-supplied packages for Gravity Forms 3.1.1.1 and Gravity Flow 3.1.0 are SHA-256-pinned as source evidence; GravityView remains package-unavailable. Gravity Forms has exactly one bounded production content-admission record for `gravityforms::frontend_runtime::shortcode:gravityform`: 41 identities from the separate 4207-message source census. The WU-003 full reviewed baseline remains provenance for that subset; seven explicit PR #15 semantic corrections define the final admitted Persian content. Gravity Flow retains exactly two bounded production content-admission records: Inbox (255 identities) and Status (43 identities), with 10 identical overlaps deduplicated to an aggregate of 288 unique admitted identities from a separate 1098-message source census. All product runtime JS handle maps remain empty and no Gravity Forms or Gravity Flow translation JSON is generated. Non-admitted identities retain the existing upstream/vendor/TranslationsPress/source-English fallback. This is partial surface admission, not full-product or browser-complete localization. See docs/LOCALIZATION.md and docs/ARCHITECTURE_CONTENT_ADMISSION_V2.md for the source/load-order and content-authority boundaries.
