...
Exact-package source provenance establishes that the Inbox task model reads current-step `get_due_date_timestamp()` both for raw AG Grid `due_date` and, separately, for `due_date_human_readable`. The step contract documents the returned due timestamp as UTC, `is_overdue()` compares that same timestamp with `time()`, and the no-due state is raw `0` plus display `-`. The bounded production adapter hooks only `gravityflow_inbox_field_value`, re-reads the current step's authoritative timestamp, and delegates the absolute instant to the existing `PGR_Jalali_Presentation`; it does not parse the human-readable display string or hook/write deadline state.

The shared current-Head WU008 exact-package WordPress/Playwright lane contains three authentic assigned Inbox fixtures: an overdue due date crossing the UTC→`Asia/Tehran` civil-day boundary, a future due date, and a no-due current step. A test-only MU plugin uses Gravity Flow's supported `gravityflow_step_due_date_timestamp` filter solely to pin deterministic due epochs across HTTP requests; PersianGravity production code never registers that filter. Enabled/disabled evidence compares authoritative due timestamp, overdue state, workflow step/final status, assignments, Inbox IDs/count, AG Grid raw compare values, ascending/descending sort and raw-due quick-filter behavior. Only `due_date_human_readable` changes to Jalali with the module enabled; disabled mode restores exact native Gravity Flow output and the no-due `-` sentinel remains unchanged.
