...
## G-008 GravityView 3.3.4 system-date qualification — 2026-09-25

PR #63 is a qualification-only batch from planning base `main@7e239fe623c20da12f909050af757b8635cbb6c7`. It adds no production GravityView adapter and does not change product/version/release metadata. Exact authority is GravityView `3.3.4` SHA-256 `af5959fb6bf0cfcb4d07d14b1933cf9ea0a9d0f994b9991f27aed47edbcb9829` plus Gravity Forms `3.1.1.1` SHA-256 `542f56ae0747f3661d1474996527298027db3fb8ed3e6469a6391aaabf61069b`.

Exact source and authentic WordPress/Playwright qualification establish separate final field-output seams for `date_created` and `date_updated`: GravityView first renders the authoritative raw Entry property through its native date formatter, then applies `gravityview/template/field/date_created/output` or `gravityview/template/field/date_updated/output` with field identity and Entry context. The disposable prototype reads the raw UTC Entry property, strict-parses it as a UTC `DateTimeImmutable`, and delegates only visible Jalali formatting to the existing `PGR_Jalali_Presentation` facade. It never parses the native display string and is removed before the pre-existing Gravity Flow regression lane.

The qualification uses `Asia/Tehran` boundary fixtures that cross local midnight and requires enabled Jalali output plus native module-disabled, `en_US`, and forced-version-drift controls. It compares DB/GFAPI/REST raw values, raw GFAPI sorting, authentic GravityView sorting, the host-native `entry_date` search path for `date_created`, row/sort-link structure, repeated rendering, and unrelated user-authored date-looking text across modes. A direct `filter_date_updated` request without a configured `date_updated` Search Bar field is recorded as the same native no-op in every mode; exhaustive optional `date_updated` search configurations are not claimed.

The registry therefore records both `gravityview.date-created` and `gravityview.date-updated` as `RUNTIME_PROVEN` with exact-version disposition `QUALIFIED_FOR_PRODUCTION_ADAPTER`, while deliberately retaining `support_state: NOT_PROVEN`, `adapter_identity: null`, and no production admission. `g008-gravityview-date-qualification.json` is the dedicated qualification artifact. The source probe now persists metadata-only provenance (path/line-range/file fingerprint/contract IDs plus boolean results), independently proves fail-closed drift for both target fields, and is structurally privacy-checked before evidence is staged for upload; raw licensed source text/windows are not part of the persisted/uploaded qualification evidence. The shared qualification-evidence reconciler binds these claims to exact PersianGravity commit/tree/package identity and to exact GravityView/Gravity Forms package identity. Exact PASS/FAIL status belongs to the resulting PR Head GitHub Actions runs; source preparation alone is not a PASS claim.
