...
Production adds one bounded `PGR_GravityView_Jalali_Presentation_Adapter` and only the exact field-specific output seams `gravityview/template/field/date_created/output` and `gravityview/template/field/date_updated/output`. The adapter requires enabled `jalali_presentation`, frontend `fa_IR`, exact GravityView plugin identity/version, exact field/context identity and a strict raw UTC Entry value. It reads the authoritative Entry property via `Template_Context::entry->as_entry()`, passes a UTC `DateTimeImmutable` to `PGR_Jalali_Presentation`, and returns native GravityView output on any unsupported/drifted condition. It never reparses the localized native display string.

The exact-package WU008 production lane proves enabled Jalali presentation for both fields with `Asia/Tehran` midnight-boundary fixtures. Separate controls prove module-disabled native output, `en_US` LTR native output and forced observed GravityView version drift to `3.3.5` returning native output. The qualification MU prototype is absent throughout production admission, and production output contains no qualification diagnostic wrapper or data attributes.

The lane also proves machine semantics remain unchanged: DB/GFAPI/REST `date_created` and `date_updated` values remain equal across modes; GFAPI and authentic GravityView sorting remain equal; the native `entry_date` filter path for `date_created` remains equal; the unconfigured direct `filter_date_updated` request remains the same native no-op; row/sort-link attributes, repeated render ordering and user-authored date-looking text remain unchanged. Exhaustive optional `date_updated` Search Bar configurations are not claimed.

The committed registry now records both `gravityview.date-created` and `gravityview.date-updated` as `RUNTIME_PROVEN + ADMITTED_VERIFIED`, with adapter identity `PGR_GravityView_Jalali_Presentation_Adapter`, runtime evidence `g008-gravityview-admission.json`, and exact-version disposition `ADMITTED_FOR_EXACT_VERSION`. The earlier `QUALIFIED_FOR_PRODUCTION_ADAPTER` state is retained only in historical PR #63 evidence, not as current support state. Source evidence remains metadata-only and privacy-checked; raw licensed source content is not persisted or uploaded.
...
For exact Gravity Flow `3.1.0`, `gravityflow.timeline-history` is `RUNTIME_PROVEN + ADMITTED_VERIFIED` for bounded Jalali calendar presentation and now also has a separately bounded `fa_IR` time-digit presentation: the already-computed host time keeps its value/timezone semantics while visible ASCII digits are shaped to Persian glyphs. Print owns no independent calendar or digit adapter; `gravityflow.print` is admitted only by inheritance when the verified Timeline renderer is reused. Native storage, raw `date_created`, note/event identity, bodies, ordering, DOM machine attributes, API/state behavior and workflow semantics remain unchanged. Module-disabled output remains exact native presentation, and English/LTR Timeline plus inherited Print remain native Gregorian with ASCII digits. Unsupported locale/formats or exact source/version/caller-chain drift fail closed to native rendering. Status `gravityflow.status.due-date` and Entry Detail `gravityflow.entry-detail.schedule` remain the exact Flow 3.1.0 residual final no-admission calendar surfaces.

PR #57 digit shaping remains a separate Entry Detail presentation concern: exact Flow 3.1.0 Entry Detail status-box visible field text may receive presentation-only Persian digits under its admitted contract. Timeline/history and Print still do not load or execute that Entry Detail adapter; their time digits use the narrower Timeline-specific exact-source/caller-chain adapter described above. Earlier dated validation sections below remain historical records of their own checkpoints; any Timeline/Print no-admission statement in those earlier sections is superseded for current repository truth by merged PR #58 and the subsequent bounded Timeline digit repair, not rewritten as if it had been true later.
...
Exact-package source establishes the host wrapper as `#gravityflow-status-box-container > #submitcomment > #minor-publishing.gravityflow-status-box`. `maybe_display_entry_detail_workflow_info()` emits Entry ID, Submitted, Last Updated, Submitted By, Status, Due and Expiration as `.gravityflow-status-box-field` human-readable nodes, then invokes `gravityflow_below_workflow_info_entry_detail`; `maybe_display_entry_detail_step_status()` follows inside the same wrapper, and the queued Scheduled/step-name path reuses the same field/value classes. There is no supported final-value server filter for the whole box. The selected seam therefore uses the host's post-workflow-info action only to enqueue a footer browser adapter; the browser adapter is exact-container scoped and mutates only visible text nodes below `.gravityflow-status-box-field`.

Digit shaping is a codepoint-only mapping `0..9 → ۰..۹`. It does not parse, recalculate or reinterpret numbers, dates, timezones, AM/PM markers, timestamps or workflow state. The existing composed `gravityflow_date_format_entry_detail → date_i18n` Jalali adapter is unchanged and remains the sole calendar-conversion owner. Visible Entry ID text is intentionally shaped; its anchor `href` and ASCII `lid` query value remain native. Scheduled/current-step text may receive digit glyph shaping when rendered in the same status box, but the exact 3.1.0 Scheduled calendar seam remains `FINAL_NO_ADMISSION_FOR_EXACT_3_1_0`. Timeline/history and Print do not load or execute the digit adapter.

Qualification is bound to the existing WU008 exact-package lane rather than a weaker substitute. Focused unit coverage tests the pure glyph map, missing-container fail-closed behavior, hidden-node exclusion, DOM attribute immutability, exact-host/version gating and non-Persian locale fallback. Authentic Playwright coverage exercises Persian enabled, Persian module-disabled and English/non-Persian enabled modes; requires Persian visible times/Entry ID in the enabled mode, ASCII time/Entry ID in the controls, exact DOM machine-attribute/control/link equality enabled vs disabled, unchanged RTL direction, unchanged raw DB/GFAPI/REST `date_created` and `workflow_timestamp`, unchanged workflow step/final status, distinct/equal due+expiration operational timestamps, equal getter counts, query/CSV isolation, and no digit adapter on Timeline or Print. The schedule branch lane compares enabled Persian glyph output against disabled native ASCII after normalizing presentation glyphs only, while retaining its calendar no-admission disposition. Exact PASS/FAIL status belongs to the resulting PR Head GitHub Actions run and is not inferred from source preparation alone.

## G-008 Gravity Flow adversarial remediation — 2026-09-23

Planning base and actual canonical base remain `main@670c9104987c067ce1e3a244864571d0e4bad4e6`; no rebind was required. Exact Gravity Flow authority remains version `3.1.0`, owner-supplied package SHA-256 `ac0573b75831380417a21a455176e25eb746d718bbbd0bb70d6da6f48cba5404`.

The review invalidated the previous blanket Entry Detail no-admission conclusion. The exact shared workflow-info format chain plus WordPress `date_i18n` forms a bounded two-hook mechanism, so Entry Detail Submitted / Last Updated / Due / Expiration are now `RUNTIME_PROVEN + ADMITTED_VERIFIED` through `PGR_Gravity_Flow_Entry_Detail_Jalali_Presentation_Adapter`. The adapter arms a request-local escaped marker only from `gravityflow_date_format_entry_detail`, acts only on that exact format at `date_i18n`, interprets the timestamp-with-offset as local Gregorian civil components via `gmdate` without a second timezone conversion, converts only the date, preserves native time text, and strips the marker on every fallback.

Authentic exact-version prototype and final production browser qualification covers different due/expiration instants, Tehran local-midnight boundaries, enabled/disabled states, forced version drift, unsupported range, forced conversion failure, repeated rendering, unrelated WordPress/GF date formatting, DB/GFAPI/REST equality, workflow/deadline/overdue/expiration state, query/CSV isolation, equal operational getter counts and zero nested getter re-entry. The affected format-hook family is exactly Submitted / Last Updated / Due / Expiration; all four were qualified because the hook cannot identify only due/expiration.

The remaining exact Flow 3.1.0 no-admission surfaces are now separate:
- Status `due_date`: direct operational getter/format/echo table path; native enabled/disabled Status proof; no table-only downstream value seam.
- Entry Detail Scheduled: explicit `date`, `date_field`, delay-relative and empty `date_field` branches authentically exercised. Type-specific native rendering and operational getter counts are unchanged, and no downstream value-only hook exists.
- Timeline initial/stored events: `gravityflow_timeline_notes` runs after `array_reverse()`. Multi-note experiments preserve storage, IDs, order, bodies and date-looking text; a display-only property is ignored while `date_created` is consumed by the renderer, so no bounded date-display property exists.
- Print: field-grid and Timeline reuse are proven; no independent date formatter exists. Workflow-sidebar dates are absent from the native Print path, not propagated.

PHP `8.2.34` is the exact qualification runtime. The prior `8.2.33` pin stopped reproducing reliably on hosted `setup-php`, so qualification infrastructure moved to `8.2.34`; production implementation was not changed by that pin, and no complete semantic-equivalence claim between patch versions is made.

Evidence is separated by class: `g008-entry-detail-admission.json` proves production admission; `g008-flow-schedule-qualification.json` proves schedule branch rejection; `g008-timeline-print-qualification.json` proves Timeline/Print behavior; `g008-flow-residual-no-admission.json` aggregates only final no-admission surfaces; source-only evidence cannot promote runtime claims. Existing Flow Inbox/Status admissions, Gravity Forms V1 and G-009/WU008 remain independent regression gates. Gravity Perks packages were not consumed by this remediation.
...
Primary evidence is `g008-flow-inbox-admission.json` plus the enabled/disabled browser/state artifacts. Registry/evidence reconciliation requires a committed `RUNTIME_PROVEN + ADMITTED_VERIFIED` Flow Inbox surface to name that runtime artifact and requires the artifact to admit the exact surface while binding the exact PersianGravity Head and exact Gravity Flow package. The existing Inbox `date_created`/`last_updated`, Status `date_created`/`workflow_timestamp`, Gravity Forms V1 and G-009 lanes remain separate regression gates. For exact Gravity Flow 3.1.0, Entry Detail Submitted / Last Updated / Due / Expiration are admitted; Status `due_date`, Entry Detail Scheduled, Timeline/history and the independent/Timeline portions of Print remain evidence-qualified `FINAL_NO_ADMISSION`. Workflow-sidebar dates are absent from Print. GravityView system dates remain deferred/unqualified by this batch.
...
## Current content-admission snapshot — 2026-09-26
...
- Gravity Forms, Gravity Flow and GravityView native JS handles: 0; generated provider translation JSON: 0. `vendor_authenticity` remains `NOT_PROVEN`; exact package byte/version admission is not a vendor-authenticity claim.
- GravityView: revision 3 `CONTENT_ADMITTED_FULL`. The 6 historical surface records still contain 467 occurrences deduplicating to the same 461 identities; one reviewed `PRODUCT_REMAINDER` admits the exact disjoint 2666 residual source-backed identities. The required source-authority union is 3127/3127 with canonical keyset `3b533294de818bd7e772533512424571c85e5aa7bccb78cfe06b8b6820645c95`, 0 fuzzy/empty/rejected/unreviewed remainder entries, 2 semantic/context review passes and 7 second-pass corrections.
- GravityView runtime projection is intentionally distinct from source admission: one exact singular/plural source collision projects 3127 admitted identities to 3126 canonical gettext runtime entries, with the alias explicitly recorded and validated rather than dropping a source identity. The dedicated G006 GravityView runtime/browser lane verifies exact GravityView 3.3.4 package identity, 3127 source authority, 3126 runtime provider entries, 127 contextual keys, 42 plural keys, zero runtime translation mismatches, provider-first/upstream-fallback behavior, frontend/admin `fa_IR` RTL, a real GravityView entry fixture and representative Persian admin labels. This is representative runtime/browser evidence, not exhaustive visual exercise of all 3127 source identities.
- The historical WU-008/WU-009 19-surface browser evidence below remains historical evidence for the earlier bounded surfaces. The current G-006 Gravity Forms and GravityView runtime lanes separately execute against the exact PR Head and exact verified vendor packages; their representative host/browser checks do not turn source-census completeness into an all-UI claim.
...
## G-006 GravityView full-content qualification

This batch closes the exact locked GravityView 3.3.4 `gk-gravityview` content census without changing the six historical surface records. Their existing 461-identity union remains the pinned accepted baseline. The new `PRODUCT_REMAINDER` is exactly 2666 disjoint source-backed identities, so the final source-authority union is exactly 3127 identities. The remainder review index binds every accepted identity to its committed translation, records two semantic/context review passes with 7 second-pass corrections, and reports 2666 accepted / 0 unreviewed / 0 rejected.

The exact source census contains one gettext lookup collision: `+[count] action` exists as a singular source identity while the singular side of the plural `+[count] action` / `+[count] actions` is a separate source identity. Admission therefore keeps both source identities, while the runtime compiler projects them to the WordPress gettext key-space with 3126 canonical runtime entries plus exact plural lookup aliases. This is an explicit source→runtime projection, not a missing translation. WordPress checks the combined singular+plural key before falling back to the singular key, so generated PHP catalogs carry both lookup forms to preserve PersianGravity provider precedence over an upstream plural answer.

`G006 GravityView Runtime` is the dedicated disposable integration/browser gate. It installs hash-verified Gravity Forms 3.1.1.1 and GravityView 3.3.4 with the exact PR Head into a real `fa_IR` WordPress runtime; validates package/version identity before extraction; proves source authority 3127 and runtime provider 3126 separately; checks all 3126 runtime mappings with 127 contextual keys, 42 plural keys and zero mismatches; verifies historical mappings plus a formerly residual identity; proves provider-first collision handling and healthy out-of-census upstream/source fallback; confirms zero GravityView native-JS authority/provider JSON; renders a real GravityView entry on the frontend and the real GravityView editor in admin; and verifies RTL plus representative Persian UI labels. Client-side `net::ERR_ABORTED` events caused by deliberate page navigation/browser teardown remain diagnostic-only, while page errors and non-aborted request failures remain blocking.

The shared WU008 licensed integration remains a required regression gate because the plural lookup fix is implemented in the shared deterministic catalog compiler. It re-exercises the exact licensed Gravity ecosystem, provider/fallback semantics, RTL/LTR controls and admitted presentation adapters to ensure the cause-level compiler fix does not regress Gravity Forms, Gravity Flow, GravityView, or the existing G-008/G-009 runtime boundaries.
