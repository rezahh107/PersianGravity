...
- Timeline/history: initial entry time is Entry `date_created`; workflow note times are Gravity Forms note `date_created`, stored in UTC. Exact Flow 3.1.0 applies `array_reverse()` before `gravityflow_timeline_notes`, then renders the filtered note array. The adversarial qualification therefore does not reject this hook merely because it exposes full note objects: it separately tests cloned display data, a separate display-only property, `date_created` representation consumption, storage equality, IDs, order and note bodies.
- Print: `Gravity_Flow_Print_Entries::render()` reuses `entry_detail_grid()` and optional `timeline()`, but exact source does not call `workflow_entry_detail_status_box()`, `maybe_display_entry_detail_workflow_info()` or `display_queued_step_details()`. Workflow-sidebar due/schedule/expiration are therefore absent/not-applicable on this Print path. Print has no independent date formatter/calendar engine; `gravityflow_print_styles` is a CSS-asset hook only.

PHP `8.2.34` is the exact qualification runtime for the current residual lane. The previous `8.2.33` pin stopped reproducing reliably on the hosted `setup-php` environment, so the test infrastructure pin moved to `8.2.34`; this did not change PersianGravity production behavior, and no complete semantic-equivalence claim between PHP patch releases is made.
