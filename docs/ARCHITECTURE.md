...
The exact Gravity Flow 3.1.0 Inbox adapter uses only `gravityflow_inbox_field_value` and recognizes the separate human-readable identities for `date_created`, `last_updated`, and `due_date`. For due dates it re-reads only the current step's authoritative `get_due_date_timestamp()` UTC Unix epoch and passes that instant through the existing facade; it never parses `due_date_human_readable` and never hooks or writes the host's due-date calculation. Gravity Flow's raw `due_date` AG Grid compare value, native raw-0/display-`-` no-due sentinel, `is_overdue()` deadline comparison, highlight state, workflow/assignment state, query/sort/filter behavior and date/date-field/delay timing calculation remain host-owned. Exact-version/source/runtime drift fails closed to native display.
