...
PersianGravity does not own Gravity Flow workflow, GravityView business behavior, SRWF business logic, fonts, payment gateways, arbitrary external plugin translations, OCR/camera scanning, online Sayad inquiry, or custom databases. G-008 owns only explicitly admitted presentation seams. Exact Flow 3.1.0 Inbox `date_created`/`last_updated`/`due_date`, Status `date_created`/`workflow_timestamp`, and Entry Detail workflow-info Submitted / Last Updated / Due / Expiration are admitted without owning Flow workflow/query/export/deadline/expiration semantics. Entry Detail calendar conversion uses only the exact shared format hook to arm a unique request-local marker and an exact-marker `date_i18n` callback to replace the date component; it never calls operational workflow getters. Separately, when `jalali_presentation` is enabled and the effective WordPress locale is exactly `fa_IR`, a dedicated Entry Detail digit-presentation adapter attaches only to `gravityflow_below_workflow_info_entry_detail` and enqueues a footer script. That script requires the exact host wrapper `#gravityflow-status-box-container > #submitcomment > #minor-publishing.gravityflow-status-box` and mutates only rendered, non-hidden text nodes below `.gravityflow-status-box-field`; it never writes attributes, URLs, `data-*`, element IDs, form/control values or server state. Visible Entry ID text is therefore shapeable while its native ASCII `lid` query value remains unchanged. Scheduled/current-step field text can receive digit glyph shaping because it shares the same exact status-box field contract, but this does not admit Scheduled calendar conversion. Exact Flow 3.1.0 Status `due_date`, Entry Detail Scheduled calendar conversion, Timeline/history initial+stored dates, and any independent Print date seam remain evidence-qualified final no-admission surfaces. Timeline and Print are outside the digit adapter; Print does not render the workflow sidebar. Global WordPress dates, ordinary GF Date fields, GravityView dates, and future Flow versions remain outside the admitted boundary.
