...
PersianGravity does not own Gravity Flow workflow, GravityView business behavior, SRWF business logic, fonts, payment gateways, arbitrary external plugin translations, OCR/camera scanning, online Sayad inquiry, or custom databases. G-008 owns only explicitly admitted presentation seams. Exact Flow 3.1.0 Inbox `date_created`/`last_updated`/`due_date`, Status `date_created`/`workflow_timestamp`, and Entry Detail workflow-info Submitted / Last Updated / Due / Expiration are admitted without owning Flow workflow/query/export/deadline/expiration semantics. Entry Detail uses only the exact shared format hook to arm a unique request-local marker and an exact-marker `date_i18n` callback to replace the date component; it never calls operational workflow getters. Exact Flow 3.1.0 Status `due_date`, Entry Detail Scheduled, Timeline/history initial+stored dates, and any independent Print date seam are evidence-qualified final no-admission surfaces. Print reuses field-grid/Timeline rendering and does not render the workflow sidebar. Global WordPress dates, ordinary GF Date fields, GravityView dates, and future Flow versions remain outside the admitted boundary.
