...
| `jalali_presentation` | source-owned Gregorian→Jalali converter, typed presentation facade, bounded Gravity Forms Entries List adapter plus exact Flow 3.1.0 Inbox/Status/Entry Detail/Timeline presentation adapters; Print inherits only the verified Timeline renderer and owns no independent calendar adapter |
...
The exact Flow 3.1.0 Entry Detail adapter admits only workflow-info Submitted / Last Updated / Due / Expiration through the qualified composed format-marker → `date_i18n` path and does not call operational deadline/expiration getters. The exact Timeline adapter admits initial/stored Timeline dates only on its qualified renderer caller chain, preserving raw `date_created`, Timeline/note identity, bodies, ordering and surrounding native time semantics. Print owns no independent calendar seam: it inherits Jalali presentation only when it reuses that verified Timeline renderer. Unsupported format/source/version/caller/context/range/conversion states fail closed to native output. Status `due_date` and Entry Detail Scheduled remain the exact Flow 3.1.0 residual calendar no-admission surfaces. Timeline and Print remain outside the separate Entry Detail Persian-digit adapter.
