...
| `jalali_presentation` | source-owned Gregorian→Jalali converter, typed presentation facade, bounded Gravity Forms Entries List adapter, exact Flow 3.1.0 Inbox/Status/Entry Detail/Timeline adapters, and exact GravityView 3.3.4 `date_created`/`date_updated` field-output adapter; Print inherits only the verified Timeline renderer and owns no independent calendar adapter |
...
bounded presentation adapters
        ...
        ├── PGR_Gravity_Flow_Entry_Detail_Jalali_Presentation_Adapter → exact Flow 3.1.0 Entry Detail Submitted / Last Updated / Due / Expiration
        ├── PGR_Gravity_Flow_Timeline_Jalali_Presentation_Adapter → exact Flow 3.1.0 Timeline/history initial + stored dates
        │       └── Gravity Flow Print → inherited verified Timeline rendering only; no independent calendar adapter
        └── PGR_GravityView_Jalali_Presentation_Adapter → exact GravityView 3.3.4 date_created / date_updated field-output seams
```
...
The exact GravityView 3.3.4 adapter registers only `gravityview/template/field/date_created/output` and `gravityview/template/field/date_updated/output`. It requires `fa_IR`, frontend context, enabled `jalali_presentation`, exact field identity, and exact active plugin version from the existing product manifest plus `gravityview/gravityview.php` header. It reads the authoritative raw Entry property via GravityView `Template_Context`, strict-parses the Gravity Forms UTC `Y-m-d H:i:s` value, and delegates only visible output to `PGR_Jalali_Presentation`; it never reparses GravityView's localized display string and never enters DB/API/query/search/sort/filter code. Module-disabled, English, malformed/missing context, and exact-version drift return native GravityView output.

The exact Flow 3.1.0 Entry Detail adapter admits only workflow-info Submitted / Last Updated / Due / Expiration through the qualified composed format-marker → `date_i18n` path and does not call operational deadline/expiration getters. The exact Timeline calendar adapter admits initial/stored Timeline dates only on its qualified renderer caller chain. The Timeline adapter keeps digit shaping in a separate row-local context bound to the same admitted note identities plus the exact `get_default_time_format()` → time `date_i18n()` caller chain and, only for `fa_IR`, shapes ASCII glyphs in the already-computed host time string. It does not parse or recalculate time. Together they preserve raw `date_created`, Timeline/note identity, bodies, ordering, timestamp/timezone semantics and host workflow state. Print owns no independent calendar or digit seam: it inherits both presentations only when it reuses that verified Timeline renderer. Unsupported locale/format/source/version/caller/context/range/conversion states fail closed to native output. Status `due_date` and Entry Detail Scheduled remain the exact Flow 3.1.0 residual calendar no-admission surfaces. Timeline and Print remain outside the separate Entry Detail Persian-digit adapter.
...
GravityView production content authority is revision 3 `CONTENT_ADMITTED_FULL`. All six accepted Surface Registry records remain independently validated and unchanged: frontend shortcode 2, Gutenberg View block 1, Admin Builder 340, Foundation Settings 41, Entry Approval 30, and Search Widget 53. Their 467 surface occurrences still deduplicate to the same deterministic 461 historical identities. One exact reviewed `PRODUCT_REMAINDER` adds the disjoint 2666 residual source-backed identities, so the locked GravityView 3.3.4 source authority is exactly 3127/3127. The remainder review is bound per identity to the committed translation, requires two semantic/context passes, and records zero unreviewed/rejected entries. One exact singular/plural source pair shares a WordPress gettext runtime lookup family; the admission model therefore keeps all 3127 source identities while the generated runtime provider contains 3126 canonical runtime entries plus evidence-bound plural lookup aliases. `gk-query-filters` remains outside the manifested/runtime-activated provider boundary. GravityView script handles remain zero and no GravityView translation JSON is generated. Upstream/vendor/TranslationsPress fallback now applies only to identities outside the admitted exact-version source census. The dedicated G006 GravityView runtime lane verifies the exact 3.3.4 package, full source/runtime census boundary, provider precedence/fallback, fa_IR RTL frontend/admin rendering, and representative Persian labels without promoting that representative browser exercise to exhaustive UI coverage.
...
Across the exact locked-source censuses, accepted Persian source coverage is now 8432/8432: Gravity Forms 4207/4207, Gravity Flow 1098/1098, and GravityView 3127/3127. Provider authority remains exact-version bounded; `action-scheduler` and `gk-query-filters` remain outside the owned runtime provider boundary, vendor/upstream plugins remain unmodified, and no native product JS handles/provider JSON are activated. The GravityView source/runtime count split (3127 admitted source identities → 3126 canonical gettext runtime entries) is an explicit evidence-bound projection, not an omission. Full accepted content authority for these exact versions does not establish ecosystem-wide, future-version, all-browser-exercised, or native-product-JS localization coverage. See `docs/ARCHITECTURE_CONTENT_ADMISSION_V2.md`, `docs/VALIDATION_CONTENT_ADMISSION_V2.md`, `docs/WU004_GRAVITYVIEW_SOURCE_ADMISSION.md`, and `docs/WU006_GRAVITYVIEW_FRONTEND_CONTENT_ADMISSION.md` for the historical bounded source/content contracts.

## Scope

PersianGravity does not own Gravity Flow workflow, GravityView business behavior, SRWF business logic, fonts, payment gateways, arbitrary external plugin translations, OCR/camera scanning, online Sayad inquiry, or custom databases. G-008 owns only explicitly admitted presentation seams. Exact Flow 3.1.0 Inbox `date_created`/`last_updated`/`due_date`, Status `date_created`/`workflow_timestamp`, Entry Detail workflow-info Submitted / Last Updated / Due / Expiration, and Timeline/history initial+stored dates are admitted without owning Flow workflow/query/export/deadline/expiration/history semantics. Entry Detail calendar conversion uses only the exact shared format hook to arm a unique request-local marker and an exact-marker `date_i18n` callback to replace the date component; it never calls operational workflow getters. Timeline conversion is exact-version/source/caller-chain bounded and preserves raw `date_created`, note/event identity, bodies, ordering and surrounding native time semantics. Print owns no independent calendar adapter and inherits presentation only when it reuses the verified Timeline renderer. Exact Flow 3.1.0 Status `due_date` and Entry Detail Scheduled calendar conversion remain evidence-qualified final no-admission surfaces; unsupported formats or exact source/version/caller-chain drift fail closed to native output. Separately, when `jalali_presentation` is enabled and the effective WordPress locale is exactly `fa_IR`, a dedicated Entry Detail digit-presentation adapter attaches only to `gravityflow_below_workflow_info_entry_detail` and enqueues a footer script. That script requires the exact host wrapper `#gravityflow-status-box-container > #submitcomment > #minor-publishing.gravityflow-status-box` and mutates only rendered, non-hidden text nodes below `.gravityflow-status-box-field`; it never writes attributes, URLs, `data-*`, element IDs, form/control values or server state. Visible Entry ID text is therefore shapeable while its native ASCII `lid` query value remains unchanged. Scheduled/current-step field text can receive digit glyph shaping because it shares the same exact status-box field contract, but this does not admit Scheduled calendar conversion. Timeline and Print remain outside this digit-shaping adapter; Print does not render the workflow sidebar. Global WordPress dates, ordinary GF Date fields, GravityView dates, and future Flow versions remain outside the admitted boundary.
...
GP File Upload Pro remains data-only at runtime: its exact source passes the relevant gettext values into the uploader through `wp_localize_script()`, so the shared PHP provider supplies the Persian labels without a new JavaScript translation subsystem. GP Advanced Select is the only G-007 product with executable compatibility behavior. `PGR_Gravity_Perks_RTL` fails closed unless exact version `1.1.21`, `fa_IR` + RTL, and the exact `gp-advanced-select-tom-select` handle all match; its CSS is further scoped to the authentic `.ts-wrapper.gfield_select.plugin-change_listener.rtl` state and uses logical inline padding to mirror the caret reserve. Exact-package WU008 browser evidence verifies that seam in RTL and verifies native no-adapter behavior in LTR. Vendor package bytes, vendor CSS, and updater behavior remain untouched.
