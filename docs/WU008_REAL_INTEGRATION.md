...
GravityView source inspection remains exact but ephemeral. The probe reads the verified installed GravityView/Gravity Forms source in memory and persists only metadata: package/Head identity, boolean contract results, relative file path, bounded line range, file SHA-256 and normalized contract IDs. It never serializes source lines, excerpts or search windows. A repository unit guard rejects any source-evidence shape outside that allowlist and independently falsifies each target field. WU008 then runs an artifact privacy verifier after source discovery, again after GravityView reconciliation, and once more before upload staging. The upload action consumes only the privacy-checked staging directory; if that guard fails, the raw working artifact directory is not uploaded.
