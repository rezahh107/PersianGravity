...
- Timeline/history remains `FINAL_NO_ADMISSION` for exact 3.1.0 for a narrower reason than the prior review. `gravityflow_timeline_notes` runs after `array_reverse()` and before rendering. Multi-note experiments prove cloned display transformation leaves storage, IDs, ordering, bodies and date-looking user text unchanged, but a separate display-only property is ignored and the renderer consumes Gregorian `date_created` directly. Final rejection therefore rests on the downstream representation contract, not on full-array access by itself.
- Print has no independent date formatter. `Gravity_Flow_Print_Entries::render()` reuses `entry_detail_grid()` and optional `timeline()`. Stored Timeline initial/event dates therefore inherit the Timeline no-admission result. Exact source/browser proof also shows Print does not invoke `workflow_entry_detail_status_box()`, `maybe_display_entry_detail_workflow_info()` or `display_queued_step_details()`; workflow-sidebar Submitted / Last Updated / Due / Scheduled / Expiration are absent/not-applicable on this Print path rather than propagated. `gravityflow_print_styles` is CSS-only.

WU008 writes separate admission, schedule, Timeline/Print and aggregate residual reconciliation artifacts. A final no-admission record is an evidence-qualified disposition for the exact 3.1.0 surface, not a claim of permanent impossibility: discovery of a new supported bounded seam even in the same exact package requires fresh qualification. Workflow deadline/schedule/expiration/history truth remains owned by Gravity Flow/Gravity Forms.
