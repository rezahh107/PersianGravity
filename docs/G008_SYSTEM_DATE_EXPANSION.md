...
## GravityView 3.3.4 system-date production admission

Exact GravityView `3.3.4` package SHA-256 `af5959fb6bf0cfcb4d07d14b1933cf9ea0a9d0f994b9991f27aed47edbcb9829` and Gravity Forms `3.1.1.1` package SHA-256 `542f56ae0747f3661d1474996527298027db3fb8ed3e6469a6391aaabf61069b` now back production admission for both `gravityview.date-created` and `gravityview.date-updated`. The production adapter is `PGR_GravityView_Jalali_Presentation_Adapter`; both registry rows are `RUNTIME_PROVEN + ADMITTED_VERIFIED` with exact-version disposition `ADMITTED_FOR_EXACT_VERSION`.

Exact source proves `DateCreated::get_content()` reads the authoritative Entry `date_created` value and formats it through `GVCommon::format_date()`, while `DateUpdated` inherits that renderer under its distinct `date_updated` identity. After native rendering, `Template_Field::field_output()` exposes the field-specific filters `gravityview/template/field/date_created/output` and `gravityview/template/field/date_updated/output` with `Template_Context`, including exact field identity and the Entry object.

The production adapter registers only those two hooks. Under enabled `jalali_presentation`, frontend `fa_IR`, exact GravityView `3.3.4`, exact field/context identity and a valid raw source, it reads the authoritative raw Entry property via `as_entry()`, strict-parses Gravity Forms' UTC `Y-m-d H:i:s` value, and delegates visible output to the existing typed `PGR_Jalali_Presentation` facade. It does not parse GravityView's already-localized display string, add diagnostic markup, mutate vendor code, or enter query/search/sort/filter/storage/API paths. Host/version, locale, module, context, field identity, raw value, range or conversion drift returns the native GravityView output.

The authentic WU008 production-admission lane runs against exact installed packages with the earlier qualification MU prototype explicitly absent. Its `Asia/Tehran` boundary fixtures cross local midnight and exercise four controls: production enabled, module disabled, `en_US`/LTR, and forced observed GravityView version drift. Enabled Persian output renders Jalali values for both fields; all three control modes remain native.

Machine semantics remain host-owned. Across those modes, DB, GFAPI and REST `date_created`/`date_updated` values remain equal; raw GFAPI sorting and authentic GravityView ascending/descending sorting remain equal; the host-native `entry_date` search path for `date_created` returns the same result set; and an unconfigured direct `filter_date_updated` request remains the same native no-op. The latter is intentionally narrow and does not claim exhaustive optional `date_updated` Search Bar configurations. Row attributes, sort links, repeated rendering, entry identity/order and user-authored date-looking text remain unchanged.

Exact vendor source remains ephemeral inside CI. Persisted source evidence is metadata-only: exact package/Head identity, boolean contract results, normalized relative paths, bounded line ranges, file SHA-256 fingerprints and contract IDs. Raw licensed source lines/windows are not staged for upload. The production admission artifact is `g008-gravityview-admission.json`; the earlier `g008-gravityview-date-qualification.json` remains the historical seam-qualification artifact.

Current exact-version dispositions:

- `gravityview.date-created`: `ADMITTED_FOR_EXACT_VERSION`
- `gravityview.date-updated`: `ADMITTED_FOR_EXACT_VERSION`
