...
- Timeline/history is `RUNTIME_PROVEN + ADMITTED_VERIFIED` for exact Flow `3.1.0` and GF `3.1.1.1` through `PGR_Gravity_Flow_Timeline_Jalali_Presentation_Adapter`. The adapter arms only on the qualified contiguous `get_note_header()` → `Gravity_Flow_Common::format_date()` → `GFCommon::format_date()` renderer path, validates the four exact source fingerprints and approved date-format profiles, and consumes a request-local one-shot marker at `date_i18n`. It replaces only the localized Gregorian date component; the authoritative local clock value/AM-PM semantics, Timeline objects, stored note data, IDs, ordering and user-authored bodies remain host-owned and unchanged. Separately, under exact `fa_IR` presentation, a header-only digit adapter may shape the already-rendered ASCII clock token to Persian glyphs after a successful qualified Timeline conversion; it does not parse or recalculate time and never traverses note bodies. Unsupported format/source/version/caller/context/range/conversion states fail closed to exact native output.
- Authentic multi-note browser evidence maps each fixture event to exactly one row-local header/body pair. The body vector must equal the authoritative fixture values in enabled and disabled Persian modes, remain identical across module state, preserve duplicate-timestamp note identity and date-looking user text, and never include the row header through wrapper nesting. Marker-leak checks remain hard gates.
- Print owns no independent date formatter or admission seam. `Gravity_Flow_Print_Entries::render()` reuses optional `Gravity_Flow_Entry_Detail::timeline()`, so `gravityflow.print` is admitted only by inheritance of the already verified Timeline renderer. Its Timeline header/body vectors must exactly match Entry Detail Timeline for the same mode, the workflow sidebar remains absent, and no independent Print conversion path is introduced.
