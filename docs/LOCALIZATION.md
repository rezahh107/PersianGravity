...
`SOURCE VERIFIED != TRANSLATION CONTENT ADMITTED != JS SURFACE ACTIVATED`. Gravity Forms and Gravity Flow are revision-3 `CONTENT_ADMITTED_FULL`. Gravity Forms preserves its six historical surface records as the exact 1759-identity accepted baseline and adds one separately reviewed `PRODUCT_REMAINDER` record for the exact disjoint 2448 residual identities; their union is the complete locked source-backed Gravity Forms 3.1.1.1 census, 4207/4207. Its review index records two semantic-review passes and 1604 second-pass corrections, with zero fuzzy, empty, rejected or unreviewed remainder entries, while the original 1759 keyset and translation-content fingerprints fail closed on drift. Gravity Flow remains unchanged at 1098/1098 (732 historical + 366 reviewed remainder, 34 second-pass corrections). Native JS handles remain zero and no Gravity Forms or Gravity Flow provider JSON is generated. GravityView remains revision-2 bounded content authority and is not broadened by this Gravity Forms change. `gk-query-filters` remains a separate bounded Composer dependency/domain that PersianGravity does not manifest or runtime-activate. `source_pot_sha256` remains `null` for metadata-only admission because no full `source.pot` is committed; for Gravity Forms/Flow `vendor_pot_sha256` records inspected vendor POT bytes, while GravityView has no vendor POT at all.
...
The canonical registry in `tools/i18n/admission/surfaces.json` contains 19 deterministic source-backed surfaces: six Gravity Forms, seven Gravity Flow and six GravityView. Classification uses explicit source-path rules only; there is no semantic fallback. GF: 4207 unique / 1759 surface-classified / 2448 outside the surface registry / 55 multi-surface. The 2448 outside the registry are now covered by the exact reviewed Gravity Forms `PRODUCT_REMAINDER` authority; classification remains unchanged because product completion does not invent new runtime surfaces. Flow: 1098 / 732 surface-classified / 366 outside the surface registry / 30 multi-surface. The 366 outside the registry remain covered by the exact reviewed Gravity Flow `PRODUCT_REMAINDER` authority. GravityView: 3127 / 461 / 2666 unclassified / 6 multi-surface. Control types are source-backed and do not imply RTL correctness.
...
Source admission does not itself populate provider translations. Current production content authority is revision-3 full for Gravity Forms and Gravity Flow, while GravityView remains revision-2 partial and record-scoped:

- Gravity Forms preserves all six historical accepted surface records unchanged. Their 1826 occurrences still deduplicate to the same 1759 identities. Revision 3 adds one exact `PRODUCT_REMAINDER` record for the disjoint 2448 residual identities, with source evidence and per-identity accepted-review hashes; 1759 + 2448 must equal the locked 4207-identity source-backed census. The known vendor-POT-only stale identity (`I'm a Button!`) remains excluded. The original 1759 keyset and translation-content fingerprints are pinned. Its script map remains empty and no Gravity Forms translation JSON is generated.
- Gravity Flow preserves all seven accepted Surface Registry records unchanged: shortcode 15, Inbox 255, Status 43, Reports 20, Form Settings / Admin Builder 391, Settings & Integrations 44, and Entry Detail Sidebar 0. Their 768 occurrences still deduplicate to 732 unique identities. Revision 3 adds one exact `PRODUCT_REMAINDER` record for the disjoint 366 identities, with source evidence plus per-identity accepted-review hashes; 732 + 366 must equal the locked 1098-identity census. Inbox and Status locked fingerprints are preserved. The zero-identity Entry Detail Sidebar record remains explicit valid authority.
- GravityView retains its separately bounded revision-2 authority at 461/3127 accepted identities; exactly 2666 source identities remain mandatory and outside provider authority.

Validated content authority is the exact union of committed admission records. Gravity Forms and Gravity Flow are revision-3 full-product authorities: Gravity Forms is 1759 historical + 2448 reviewed remainder = 4207/4207, and Gravity Flow remains 732 historical + 366 reviewed remainder = 1098/1098. GravityView remains revision-2 partial authority at 461/3127. Total accepted locked-source coverage is 5766/8432; exactly 2666 GravityView identities remain mandatory, so G-006 remains active. Product/source identity alone is never authority. All product script maps remain empty; Gravity Forms and Gravity Flow activate zero native JS translation handles and generate no provider translation JSON. Missing identities in products that remain partial retain upstream/vendor/TranslationsPress fallback and then source English. Full-product content admission is a translation-content claim; representative licensed runtime evidence is reported separately in `docs/VALIDATION.md`.
