...
`SOURCE VERIFIED != TRANSLATION CONTENT ADMITTED != JS SURFACE ACTIVATED`. Gravity Forms remains at revision-2 production content authority for six independently accepted records with a deterministic 1759-identity aggregate from the separate 4207-message source census. Gravity Flow is now revision-3 `CONTENT_ADMITTED_FULL`: the seven historical surface records still union to the same 732 accepted identities, and a separately reviewed `PRODUCT_REMAINDER` record admits the exact remaining 366 identities. The disjoint union is therefore the complete locked Gravity Flow 3.1.0 census, 1098/1098, with zero fuzzy, empty, rejected or unreviewed remainder entries. The remainder review index records two semantic-review passes and 34 second-pass corrections; preexisting keyset and translation-content fingerprints fail closed if any of the original 732 translations drift. Native JS handles remain zero and no Gravity Flow translation JSON is generated. GravityView remains revision-2 bounded content authority and is not broadened by this Gravity Flow change. `gk-query-filters` remains a separate bounded Composer dependency/domain that PersianGravity does not manifest or runtime-activate. `source_pot_sha256` remains `null` for metadata-only admission because no full `source.pot` is committed; for Gravity Forms/Flow `vendor_pot_sha256` records inspected vendor POT bytes, while GravityView has no vendor POT at all.
...
The canonical registry in `tools/i18n/admission/surfaces.json` contains 19 deterministic source-backed surfaces: six Gravity Forms, seven Gravity Flow and six GravityView. Classification uses explicit source-path rules only; there is no semantic fallback. GF: 4207 unique / 1759 classified / 2448 unclassified / 55 multi-surface. Flow: 1098 / 732 surface-classified / 366 outside the surface registry / 30 multi-surface. The 366 outside the registry are now covered by the exact reviewed `PRODUCT_REMAINDER` authority; classification remains unchanged because product completion does not invent new runtime surfaces. GravityView: 3127 / 461 / 2666 unclassified / 6 multi-surface. Control types are source-backed and do not imply RTL correctness.
...
Validated content authority is the exact union of committed admission records. Gravity Forms and GravityView remain revision-2 bounded authorities. Gravity Flow revision 3 is the first full-product authority: its unchanged 732 historical surface identities plus its reviewed 366-identity product remainder must exactly equal the locked 1098 source identities. Product/source identity alone is never authority. All product script maps remain empty, Gravity Flow activates zero native JS translation handles, and no Gravity Flow translation JSON is generated. Missing identities in products that remain partial still retain upstream/vendor/TranslationsPress fallback and then source English. Full Gravity Flow content admission is a translation-content claim; licensed runtime/browser evidence is reported separately in `docs/VALIDATION.md`.
