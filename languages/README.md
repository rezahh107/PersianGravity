...
All three G-007 native WordPress script-translation maps are empty and no G-007 provider translation JSON is generated. File Upload Pro's uploader strings are PHP-gettext values passed through `wp_localize_script()`, so its Persian labels use the existing PHP provider path rather than a parallel JavaScript catalog. GP Advanced Select has one bounded compatibility adapter gated to exact 1.1.21 + `fa_IR`/RTL + the exact `gp-advanced-select-tom-select` style handle and scoped to the authentic `gfield_select` / `plugin-change_listener` RTL wrapper; it does not edit vendor files or updater behavior.
...
Exact package/source/build evidence remains the localization authority, and current G-009/WU008 exact-package browser evidence separately verifies GP File Upload Pro 1.5.13's visible uploader labels/RTL behavior plus GP Advanced Select 1.1.21 caret/padding and native LTR control. This does not admit any unlisted Perk or future version. See `docs/LOCALIZATION.md`, `docs/G007_GRAVITY_PERKS_LOCALIZATION.md`, `docs/VALIDATION_G007.md`, `docs/G009_RTL_BIDI_COMPATIBILITY.md`, and `docs/ARCHITECTURE_CONTENT_ADMISSION_V2.md`.
